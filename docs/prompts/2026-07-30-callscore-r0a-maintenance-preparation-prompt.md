...
- immutable pre-worktree bootstrap inputs:
  - `scripts/callscore-r0a-bootstrap.py` — `90ec9e244461987097bd32583293da858de55e2de5693155cb460a6417e311f7`;
  - `tests/test_callscore_r0a_bootstrap.py` — `e918c28c4b7a40ade5e7b1dc8b80e9e40ad89d656705d5ad9f86006e7d08e5ea`;
...
```bash
/usr/bin/env -i HOME=/nonexistent PATH=/usr/bin:/bin LANG=C.UTF-8 LC_ALL=C.UTF-8 GIT_OPTIONAL_LOCKS=0 GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_AUTHOR_NAME='CallScore R0A' GIT_AUTHOR_EMAIL='r0a@call-score.invalid' GIT_COMMITTER_NAME='CallScore R0A' GIT_COMMITTER_EMAIL='r0a@call-score.invalid' /usr/bin/git -c core.hooksPath="$HOOKS_DIR" -c core.fsmonitor=false -c core.untrackedCache=false <reviewed-subcommand-and-argv>
```
...
/usr/bin/mkdir -m 0700 -- "$HOOKS_DIR"
...
/usr/bin/env -i HOME=/nonexistent PATH=/usr/bin:/bin LANG=C.UTF-8 LC_ALL=C.UTF-8 GIT_OPTIONAL_LOCKS=0 GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_AUTHOR_NAME='CallScore R0A' GIT_AUTHOR_EMAIL='r0a@call-score.invalid' GIT_COMMITTER_NAME='CallScore R0A' GIT_COMMITTER_EMAIL='r0a@call-score.invalid' /usr/bin/git -C /srv/agents/repos/callscore-workplane -c core.hooksPath="$HOOKS_DIR" -c core.fsmonitor=false -c core.untrackedCache=false worktree add -b "r0a/hermes-state-maintenance-$R0A_NONCE" "$WORKPLANE_WORKTREE" "$WORKPLANE_BASE_COMMIT"
/usr/bin/test -z "$(/usr/bin/find "$HOOKS_DIR" -mindepth 1 -maxdepth 1 -print -quit)"
/usr/bin/env -i HOME=/nonexistent PATH=/usr/bin:/bin LANG=C.UTF-8 LC_ALL=C.UTF-8 GIT_OPTIONAL_LOCKS=0 GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_AUTHOR_NAME='CallScore R0A' GIT_AUTHOR_EMAIL='r0a@call-score.invalid' GIT_COMMITTER_NAME='CallScore R0A' GIT_COMMITTER_EMAIL='r0a@call-score.invalid' /usr/bin/git -C /opt/crypto-tuber-ranked -c core.hooksPath="$HOOKS_DIR" -c core.fsmonitor=false -c core.untrackedCache=false worktree add -b "r0a/callscore-maintenance-artifacts-$R0A_NONCE" "$APP_WORKTREE" "$APP_BASE_COMMIT"
```

Input-manifest generation uses the reviewed bootstrap module's `capture_file`, `canonical_bytes`, `validate_manifest`, `validate_manifest_identity` and `validate_manifest_files` functions with the exact tuple/path/hash values above; the resulting bytes have no trailing LF. The duplicate-key-free, float-free, printable-ASCII, safe-integer-bounded bootstrap runtime validator is authoritative; the committed JSON Schema is a parity artifact and its negative matrix must prove that it accepts and rejects the same boundary objects. No mutable third-party schema package and no later-created Workplane/application validator is part of this pre-worktree gate.
...
On the first failed or blocked gate, stop issuing implementation and Git-mutation commands. If the nonce control root was created and remains descriptor/identity-valid, atomically create one owner-only, value-free JCS failure receipt there, bind the failed command/audit reference and hashes, then make no further source or Git mutation. Preserve the nonce worktrees, exact nonce branches, captures, traces and failure receipt for forensic review; do not automatically remove, reset, prune or reuse them. A failed nonce is never reused. A clean rerun requires a newly reviewed nonce and fresh non-pre-existing paths, so retained state cannot collide with or authorise the rerun. Cleanup of a failed nonce is a separate, explicitly reviewed source-only operation and is not part of R0A. If the control root itself is untrusted or unavailable, return a parent-side blocker without creating the failure receipt.
