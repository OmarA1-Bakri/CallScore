...
- immutable pre-worktree bootstrap inputs:
  - `scripts/callscore-r0a-bootstrap.py` — `acd8675e62664b93ceafcb3c7abfa02eb45495f9dc592332249b7db00dae1806`;
  - `tests/test_callscore_r0a_bootstrap.py` — `db807812075750a19ae7fc4449238da70c8f0a49f2606eb35b9c3846364e731e`;
...
`R0A_GIT_ADMIN_ALLOWLIST` is the only exception to the ordinary two-worktree/control/fixture write roots. For each repository it contains only the exact nonce branch ref, its transient `.lock`, the exact nonce reflog and its transient `.lock`, the exact linked-worktree administration directory selected by `git rev-parse --git-path`, and the parent `worktrees/`, `refs/heads/r0a/` and `logs/refs/heads/r0a/` directories only when Git must create them. The shared `.git/objects/` directory is read-only during pre-worktree creation: creation, modification, replacement or deletion of any loose object, pack, index or bitmap byte blocks. Later reviewed commit commands may create new content-addressed objects only after binding the complete pre-existing object membership/device/inode/hash state; every pre-existing object must remain byte- and identity-stable, every new object must hash to its pathname and be reachable from the exact nonce branch commit, and no pack/index replacement is allowed. Parent-directory exceptions are create-only and pre/post membership must show only the exact nonce child. Snapshot device/inode/hash/member state before and after every Git mutation. The evidence harness must prove that every shared-administration write is explained by the literal worktree/add/commit argv, that only the exact nonce branch moved, and that no hook, filter, maintenance, GC, fsmonitor or external index process executed. Any other shared `.git` mutation blocks R0A.
...
The descriptor-safe `prepare-private-directory` creation and owner/device/inode verification of `HOOKS_DIR` and `CONTROL_ROOT` is the irreducible immutable bootstrap TCB: it opens `/srv/agents/worktrees` once, uses `mkdirat`/`openat(O_NOFOLLOW)`, performs fd/path identity checks, and creates only those two empty private directories. The exact committed bootstrap bytes and `/usr/bin/strace` launch/audit path are also TCB because tracing the tracer/auditor would recurse. After that root exists, every evidence-producing, artifact-producing, Git, manifest, test and failure-receipt command must execute through `run-audited`; its target receives only the fixed minimal environment, stdin `/dev/null`, create-only owner-private stdout/stderr captures, `strace -ff -qq -ttt -s 4096 -yy -e trace=all`, exact-mode `0600` trace shards, paired unfinished/resumed-record normalisation, cross-shard clone/cwd/mapping inheritance, complete-shard validation, pre/post-fstat plus final-path identity checks, an explicit execve allowlist, the originally recorded control-root device/inode, and the exact write allowlist. A TCB setup failure or failure-receipt write/audit failure emits no success/failure receipt and must be reported only as `bootstrap_blocked_unreceipted`; it cannot authorise cleanup, retry reuse or any later phase.
...
HOOKS_ID="$(/usr/bin/python3 scripts/callscore-r0a-bootstrap.py prepare-private-directory --parent /srv/agents/worktrees --name ".r0a-empty-hooks-$R0A_NONCE")"
CONTROL_ID="$(/usr/bin/python3 scripts/callscore-r0a-bootstrap.py prepare-private-directory --parent /srv/agents/worktrees --name ".r0a-control-$R0A_NONCE")"
HOOKS_DEVICE="${HOOKS_ID%%:*}"; HOOKS_INODE="${HOOKS_ID#*:}"
CONTROL_DEVICE="${CONTROL_ID%%:*}"; CONTROL_INODE="${CONTROL_ID#*:}"
for R0A_ID_COMPONENT in "$HOOKS_DEVICE" "$HOOKS_INODE" "$CONTROL_DEVICE" "$CONTROL_INODE"; do /usr/bin/printf '%s' "$R0A_ID_COMPONENT" | /usr/bin/grep -Eq '^[0-9]+$'; done
r0a_failure() { rc=$?; trap - ERR; /usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/failure-receipt.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" --allowed-write-path "$CONTROL_ROOT/failure-receipt.json" -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py write-failure-receipt --control-root "$CONTROL_ROOT" --control-device "$CONTROL_DEVICE" --control-inode "$CONTROL_INODE" --nonce "$R0A_NONCE" --phase preworktree --command-id "$R0A_COMMAND_ID" --exit-code "$rc" --bootstrap-sha256 acd8675e62664b93ceafcb3c7abfa02eb45495f9dc592332249b7db00dae1806 --test-sha256 db807812075750a19ae7fc4449238da70c8f0a49f2606eb35b9c3846364e731e --schema-sha256 1e281a113cfa61b43bda6d3a192deb1907d1238c0cea0473d9c4f24a0fb99213 || exit 126; exit "$rc"; }
trap r0a_failure ERR
R0A_COMMAND_ID=immutable-hash-check
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/immutable-hash.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py verify-pins --pin acd8675e62664b93ceafcb3c7abfa02eb45495f9dc592332249b7db00dae1806=/opt/crypto-tuber-ranked/scripts/callscore-r0a-bootstrap.py --pin db807812075750a19ae7fc4449238da70c8f0a49f2606eb35b9c3846364e731e=/opt/crypto-tuber-ranked/tests/test_callscore_r0a_bootstrap.py --pin 1e281a113cfa61b43bda6d3a192deb1907d1238c0cea0473d9c4f24a0fb99213=/opt/crypto-tuber-ranked/docs/ops/callscore-r0a/bootstrap/callscore-r0a-input-manifest-v1.schema.json --pin 085630cb45fcf842d53de1270b2d2a88c6ed29b9b6dab0189d8711e638c1a492=/opt/crypto-tuber-ranked/docs/ops/callscore-r0a/bootstrap/input-spec.json
R0A_COMMAND_ID=fixture-root-create
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/fixture-root.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" -- /usr/bin/mkdir -m 0700 -- "$CONTROL_ROOT/fixtures"
R0A_COMMAND_ID=bootstrap-unit-tests
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/bootstrap-tests.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" --allowed-fixture-root "$CONTROL_ROOT/fixtures" --allowed-exec /usr/bin/git --allowed-exec /usr/lib/git-core/git --env "TMPDIR=$CONTROL_ROOT/fixtures" -- /usr/bin/python3 -m unittest -v tests/test_callscore_r0a_bootstrap.py
R0A_COMMAND_ID=input-manifest-create
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/manifest-create.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" --allowed-write-root "/srv/agents/worktrees/.r0a-input-captures-$R0A_NONCE" --allowed-write-path "/srv/agents/worktrees/.r0a-input-manifest-$R0A_NONCE.json" --allowed-exec /usr/bin/git --allowed-exec /usr/lib/git-core/git -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py create --spec /opt/crypto-tuber-ranked/docs/ops/callscore-r0a/bootstrap/input-spec.json --output "/srv/agents/worktrees/.r0a-input-manifest-$R0A_NONCE.json" --capture-dir "/srv/agents/worktrees/.r0a-input-captures-$R0A_NONCE" --hooks-dir "$HOOKS_DIR" --application-repo /opt/crypto-tuber-ranked --workplane-repo /srv/agents/repos/callscore-workplane --hermes-repo /srv/agents/hermes/hermes-agent --application-commit "$APP_BASE_COMMIT" --application-tree "$APP_BASE_TREE" --workplane-commit "$WORKPLANE_BASE_COMMIT" --workplane-tree "$WORKPLANE_BASE_TREE" --hermes-commit 3c388db06b6543821f15ed62efb9d8e7cd9bb9be --hermes-tree ad9824f4ba7e007b42bf6aa2fd171df6916790a8 --plan-sha256 "$PLAN_SHA256" --prompt-sha256 "$R0A_PROMPT_SHA256"
R0A_COMMAND_ID=input-manifest-validate
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/manifest-validate.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" --allowed-exec /usr/bin/git --allowed-exec /usr/lib/git-core/git -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py validate --manifest "/srv/agents/worktrees/.r0a-input-manifest-$R0A_NONCE.json"
/usr/bin/test -z "$(/usr/bin/find "$HOOKS_DIR" -mindepth 1 -maxdepth 1 -print -quit)"
R0A_COMMAND_ID=workplane-worktree-add
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/workplane-add.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /srv/agents/repos/callscore-workplane --allowed-write-root "$WORKPLANE_WORKTREE" --allowed-write-root "$WORKPLANE_GIT_DIR/worktrees/$WORKPLANE_ADMIN_NAME" --allowed-write-path "$WORKPLANE_GIT_DIR/worktrees" --allowed-write-path "$WORKPLANE_GIT_DIR/refs/heads/r0a" --allowed-write-path "$WORKPLANE_GIT_DIR/logs/refs/heads/r0a" --allowed-write-path "$WORKPLANE_GIT_DIR/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE" --allowed-write-path "$WORKPLANE_GIT_DIR/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE.lock" --allowed-write-path "$WORKPLANE_GIT_DIR/logs/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE" --allowed-write-path "$WORKPLANE_GIT_DIR/logs/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE.lock" --allowed-exec /usr/bin/git --allowed-exec /usr/lib/git-core/git -- /usr/bin/env GIT_OPTIONAL_LOCKS=0 GIT_TERMINAL_PROMPT=0 GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_PAGER=/usr/bin/cat GIT_AUTHOR_NAME='CallScore R0A' GIT_AUTHOR_EMAIL='r0a@call-score.invalid' GIT_COMMITTER_NAME='CallScore R0A' GIT_COMMITTER_EMAIL='r0a@call-score.invalid' /usr/bin/git -c core.hooksPath="$HOOKS_DIR" -c core.fsmonitor=false -c core.untrackedCache=false worktree add -b "r0a/hermes-state-maintenance-$R0A_NONCE" "$WORKPLANE_WORKTREE" "$WORKPLANE_BASE_COMMIT"
...
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/application-add.trace" --trace-parent-device "$CONTROL_DEVICE" --trace-parent-inode "$CONTROL_INODE" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$APP_WORKTREE" --allowed-write-root "$APP_GIT_DIR/worktrees/$APP_ADMIN_NAME" --allowed-write-path "$APP_GIT_DIR/worktrees" --allowed-write-path "$APP_GIT_DIR/refs/heads/r0a" --allowed-write-path "$APP_GIT_DIR/logs/refs/heads/r0a" --allowed-write-path "$APP_GIT_DIR/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE" --allowed-write-path "$APP_GIT_DIR/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE.lock" --allowed-write-path "$APP_GIT_DIR/logs/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE" --allowed-write-path "$APP_GIT_DIR/logs/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE.lock" --allowed-exec /usr/bin/git --allowed-exec /usr/lib/git-core/git -- /usr/bin/env GIT_OPTIONAL_LOCKS=0 GIT_TERMINAL_PROMPT=0 GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_PAGER=/usr/bin/cat GIT_AUTHOR_NAME='CallScore R0A' GIT_AUTHOR_EMAIL='r0a@call-score.invalid' GIT_COMMITTER_NAME='CallScore R0A' GIT_COMMITTER_EMAIL='r0a@call-score.invalid' /usr/bin/git -c core.hooksPath="$HOOKS_DIR" -c core.fsmonitor=false -c core.untrackedCache=false worktree add -b "r0a/callscore-maintenance-artifacts-$R0A_NONCE" "$APP_WORKTREE" "$APP_BASE_COMMIT"
...
`r0a_evidence.py` must run with a cleared/minimal environment containing only `HOME=/nonexistent`, `PATH=/usr/bin:/bin`, `LANG=C.UTF-8`, `LC_ALL=C.UTF-8`, `PYTHONDONTWRITEBYTECODE=1`, `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_TERMINAL_PROMPT=0`, `GIT_OPTIONAL_LOCKS=0`, `GIT_PAGER=/usr/bin/cat` and command-specific value-free bindings; fixture-only temp roots and no network. It launches every post-worktree command under `strace -ff -qq -ttt -s 4096 -yy -e trace=all`, with `close_fds=true`, stdin from `/dev/null`, and only captured stdout/stderr fds inherited. The audit rejects every network syscall (including AF_UNIX), `ptrace`, namespace/mount/BPF escape, external-process signal, unresolved relative dirfd, unresolved inherited write fd, Git argv without exactly one final pinned empty `core.hooksPath`, and every write/mutation outside the two nonce worktrees, nonce control/evidence paths, declared owner-only fixture roots and `R0A_GIT_ADMIN_ALLOWLIST`. Successful `chdir`/`fchdir` is tracked per trace from the successful syscall and kernel `-yy` fd/cwd annotations; the post-run auditor never re-resolves a now-deleted fixture path through the live filesystem. Cwd movement grants no write authority, and every later relative mutation uses its kernel-resolved `AT_FDCWD<...>`/returned-fd target and must still land in the exact write allowlist. Unresolved cwd or dirfd identity blocks. Mutating opens require the `-yy` kernel-resolved returned-fd target; lexical paths are never post-resolved through mutable symlinks. All write roots are owner-only, descriptor-bound and symlink-free before and after each command; symlink creation is forbidden outside an explicitly declared disposable fixture root, and a fixture symlink never grants authority to any resolved target outside that fixture. It records one audit object per process tree. Missing `strace`, a truncated/unparsed trace, unknown fd/path identity or a write outside the allowlist makes the associated mutation observations `unknown|true` and blocks `prepared`.
...
On the first failed or blocked gate, stop issuing implementation and Git-mutation commands. The exact receipt path is `/srv/agents/worktrees/.r0a-control-$R0A_NONCE/failure-receipt.json`; it is created exclusively by `scripts/callscore-r0a-bootstrap.py write-failure-receipt` running as a `run-audited` target. Its exact JCS schema is `callscore.r0a_failure_receipt.v1` with only `schema`, `nonce`, `phase`, `command_id`, `exit_code`, `bootstrap_sha256`, `test_sha256`, and `manifest_schema_sha256`. The pre-worktree `ERR` trap is installed immediately after the owner-private control-root TCB verification and before the audited immutable-hash check. The trap clears itself before launching the audited create-only failure writer, and every mutating stage sets a fixed ASCII `R0A_COMMAND_ID`. The subcommand reopens the canonical nonce control root with `O_NOFOLLOW`, revalidates owner/mode/device/inode, and publishes the receipt create-only with mode `0600`; an existing receipt fails closed. If the bootstrap hash check fails, the receipt records the reviewed expected bootstrap/test/schema hashes and the fixed `immutable-hash-check` command ID; it does not claim those current bytes were trusted. If the failure writer, its trace, or its trace audit fails, report only `bootstrap_blocked_unreceipted` and preserve every partial file. Preserve nonce worktrees, exact nonce branches, captures, traces and receipt for forensic review; do not automatically remove, reset, prune or reuse them. A failed nonce is never reused. A clean rerun requires a newly reviewed nonce and fresh non-pre-existing paths. Cleanup is a separate explicitly reviewed source-only operation.
