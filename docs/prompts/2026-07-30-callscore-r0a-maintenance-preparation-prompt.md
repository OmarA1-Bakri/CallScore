...
- immutable pre-worktree bootstrap inputs:
  - `scripts/callscore-r0a-bootstrap.py` — `d79e6c6a085e794959dec8bf4c78055e8e203dcb3de104bc57f19be4ce898aad`;
  - `tests/test_callscore_r0a_bootstrap.py` — `f5a7c2229ce350c7af540f5f669d09bd0741cb8e6c4d818a8a63ed9c7811af25`;
...
The descriptor-safe creation and owner/device/inode verification of `HOOKS_DIR` and `CONTROL_ROOT` is the irreducible immutable bootstrap TCB: it performs no repository/provider/network action and creates only those two empty private directories. The exact committed bootstrap bytes and `/usr/bin/strace` launch/audit path are also TCB because tracing the tracer/auditor would recurse. After that root exists, every evidence-producing, artifact-producing, Git, manifest, test and failure-receipt command must execute through `run-audited`; its target receives only the fixed minimal environment, stdin `/dev/null`, create-only owner-private stdout/stderr captures, `strace -ff -qq -s 4096 -yy -e trace=all`, complete-shard validation and the exact write allowlist. A TCB setup failure or failure-receipt write/audit failure emits no success/failure receipt and must be reported only as `bootstrap_blocked_unreceipted`; it cannot authorize cleanup, retry reuse or any later phase.
...
r0a_failure() { rc=$?; trap - ERR; /usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/failure-receipt.trace" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" --allowed-write-path "$CONTROL_ROOT/failure-receipt.json" -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py write-failure-receipt --control-root "$CONTROL_ROOT" --nonce "$R0A_NONCE" --phase preworktree --command-id "$R0A_COMMAND_ID" --exit-code "$rc" --bootstrap-sha256 d79e6c6a085e794959dec8bf4c78055e8e203dcb3de104bc57f19be4ce898aad --test-sha256 f5a7c2229ce350c7af540f5f669d09bd0741cb8e6c4d818a8a63ed9c7811af25 --schema-sha256 1e281a113cfa61b43bda6d3a192deb1907d1238c0cea0473d9c4f24a0fb99213 || exit 126; exit "$rc"; }
trap r0a_failure ERR
R0A_COMMAND_ID=immutable-hash-check
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/immutable-hash.trace" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py verify-pins --pin d79e6c6a085e794959dec8bf4c78055e8e203dcb3de104bc57f19be4ce898aad=/opt/crypto-tuber-ranked/scripts/callscore-r0a-bootstrap.py --pin f5a7c2229ce350c7af540f5f669d09bd0741cb8e6c4d818a8a63ed9c7811af25=/opt/crypto-tuber-ranked/tests/test_callscore_r0a_bootstrap.py --pin 1e281a113cfa61b43bda6d3a192deb1907d1238c0cea0473d9c4f24a0fb99213=/opt/crypto-tuber-ranked/docs/ops/callscore-r0a/bootstrap/callscore-r0a-input-manifest-v1.schema.json --pin 085630cb45fcf842d53de1270b2d2a88c6ed29b9b6dab0189d8711e638c1a492=/opt/crypto-tuber-ranked/docs/ops/callscore-r0a/bootstrap/input-spec.json
R0A_COMMAND_ID=fixture-root-create
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/fixture-root.trace" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" -- /usr/bin/mkdir -m 0700 -- "$CONTROL_ROOT/fixtures"
R0A_COMMAND_ID=bootstrap-unit-tests
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/bootstrap-tests.trace" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" --allowed-fixture-root "$CONTROL_ROOT/fixtures" --env "TMPDIR=$CONTROL_ROOT/fixtures" -- /usr/bin/python3 -m unittest -v tests/test_callscore_r0a_bootstrap.py
R0A_COMMAND_ID=input-manifest-create
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/manifest-create.trace" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" --allowed-write-root "/srv/agents/worktrees/.r0a-input-captures-$R0A_NONCE" --allowed-write-path "/srv/agents/worktrees/.r0a-input-manifest-$R0A_NONCE.json" -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py create --spec /opt/crypto-tuber-ranked/docs/ops/callscore-r0a/bootstrap/input-spec.json --output "/srv/agents/worktrees/.r0a-input-manifest-$R0A_NONCE.json" --capture-dir "/srv/agents/worktrees/.r0a-input-captures-$R0A_NONCE" --hooks-dir "$HOOKS_DIR" --application-repo /opt/crypto-tuber-ranked --workplane-repo /srv/agents/repos/callscore-workplane --hermes-repo /srv/agents/hermes/hermes-agent --application-commit "$APP_BASE_COMMIT" --application-tree "$APP_BASE_TREE" --workplane-commit "$WORKPLANE_BASE_COMMIT" --workplane-tree "$WORKPLANE_BASE_TREE" --hermes-commit 3c388db06b6543821f15ed62efb9d8e7cd9bb9be --hermes-tree ad9824f4ba7e007b42bf6aa2fd171df6916790a8 --plan-sha256 "$PLAN_SHA256" --prompt-sha256 "$R0A_PROMPT_SHA256"
R0A_COMMAND_ID=input-manifest-validate
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/manifest-validate.trace" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$CONTROL_ROOT" -- /usr/bin/python3 scripts/callscore-r0a-bootstrap.py validate --manifest "/srv/agents/worktrees/.r0a-input-manifest-$R0A_NONCE.json"
/usr/bin/test -z "$(/usr/bin/find "$HOOKS_DIR" -mindepth 1 -maxdepth 1 -print -quit)"
R0A_COMMAND_ID=workplane-worktree-add
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/workplane-add.trace" --cwd /srv/agents/repos/callscore-workplane --allowed-write-root "$WORKPLANE_WORKTREE" --allowed-write-root "$WORKPLANE_GIT_DIR/objects" --allowed-write-root "$WORKPLANE_GIT_DIR/worktrees/$WORKPLANE_ADMIN_NAME" --allowed-write-path "$WORKPLANE_GIT_DIR/worktrees" --allowed-write-path "$WORKPLANE_GIT_DIR/refs/heads/r0a" --allowed-write-path "$WORKPLANE_GIT_DIR/logs/refs/heads/r0a" --allowed-write-path "$WORKPLANE_GIT_DIR/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE" --allowed-write-path "$WORKPLANE_GIT_DIR/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE.lock" --allowed-write-path "$WORKPLANE_GIT_DIR/logs/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE" --allowed-write-path "$WORKPLANE_GIT_DIR/logs/refs/heads/r0a/hermes-state-maintenance-$R0A_NONCE.lock" -- /usr/bin/env GIT_OPTIONAL_LOCKS=0 GIT_TERMINAL_PROMPT=0 GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_PAGER=/usr/bin/cat GIT_AUTHOR_NAME='CallScore R0A' GIT_AUTHOR_EMAIL='r0a@call-score.invalid' GIT_COMMITTER_NAME='CallScore R0A' GIT_COMMITTER_EMAIL='r0a@call-score.invalid' /usr/bin/git -c core.hooksPath="$HOOKS_DIR" -c core.fsmonitor=false -c core.untrackedCache=false worktree add -b "r0a/hermes-state-maintenance-$R0A_NONCE" "$WORKPLANE_WORKTREE" "$WORKPLANE_BASE_COMMIT"
...
/usr/bin/python3 scripts/callscore-r0a-bootstrap.py run-audited --trace-prefix "$CONTROL_ROOT/application-add.trace" --cwd /opt/crypto-tuber-ranked --allowed-write-root "$APP_WORKTREE" --allowed-write-root "$APP_GIT_DIR/objects" --allowed-write-root "$APP_GIT_DIR/worktrees/$APP_ADMIN_NAME" --allowed-write-path "$APP_GIT_DIR/worktrees" --allowed-write-path "$APP_GIT_DIR/refs/heads/r0a" --allowed-write-path "$APP_GIT_DIR/logs/refs/heads/r0a" --allowed-write-path "$APP_GIT_DIR/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE" --allowed-write-path "$APP_GIT_DIR/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE.lock" --allowed-write-path "$APP_GIT_DIR/logs/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE" --allowed-write-path "$APP_GIT_DIR/logs/refs/heads/r0a/callscore-maintenance-artifacts-$R0A_NONCE.lock" -- /usr/bin/env GIT_OPTIONAL_LOCKS=0 GIT_TERMINAL_PROMPT=0 GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_PAGER=/usr/bin/cat GIT_AUTHOR_NAME='CallScore R0A' GIT_AUTHOR_EMAIL='r0a@call-score.invalid' GIT_COMMITTER_NAME='CallScore R0A' GIT_COMMITTER_EMAIL='r0a@call-score.invalid' /usr/bin/git -c core.hooksPath="$HOOKS_DIR" -c core.fsmonitor=false -c core.untrackedCache=false worktree add -b "r0a/callscore-maintenance-artifacts-$R0A_NONCE" "$APP_WORKTREE" "$APP_BASE_COMMIT"
