...
> agent roster. Current canonical runtime uses 51 agents/souls in
...
> Historical P12 status, superseded by O13/canonical-green evidence. Current canonical runtime is 51 agents/souls in `docs/ops/callscore-channel-head-souls.yaml` (44 retained baseline agents plus 7 justified YouTube production agents), graph-backed O13 entrypoints, and full-suite baseline 1058/1058 at `50501ad` plus later remediation commits. Treat older 8-agent/task counts below as P12 historical receipt values only, not current operating expectations.
...
| Heartbeat CLI `--dry-run` | BUILT | Historical P12 parent run: `npm run agents:heartbeat -- --dry-run` PASS, `db_write_performed:false`, 8 core channel-head tasks proposed. Current canonical dry-run should report 51 canonical agents/souls. | Safe local/no-DB-write lane for exercising heartbeat plan and receipts. |
| Heartbeat CLI `--no-db-write` | BUILT | Historical P12 parent run: `npm run agents:heartbeat -- --no-db-write` PASS, `db_write_performed:false`, 8 core channel-head tasks proposed. Current canonical dry-run should report 51 canonical agents/souls. | Alias for dry-run/no-DB-write. |
