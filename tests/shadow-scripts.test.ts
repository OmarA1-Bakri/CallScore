...
import { main as promoteShadowMain, parsePromoteShadowArgs } from "../src/scripts/promote-shadow-extractions";

test("shadow extraction defaults to all transcript recheck selection without production writes", () => {
  const args = parseShadowExtractArgs(["--run-id", "shadow-test", "--limit", "5"]);
...
test("shadow promotion requires explicit run and status guardrails", () => {
  ...
  assert.throws(
    () => parsePromoteShadowArgs(["--shadow-in", "s.jsonl", "--diff-in", "d.jsonl"]),
...
test("shadow promotion emits structured dry-run audit rows before any write", async () => {
  ...
  writeFileSync(shadowIn, `${JSON.stringify({
  ...
  writeFileSync(diffIn, `${JSON.stringify({
  ...
  const rows = readFileSync(auditOut, "utf8").trim().split(/\r?\n/).map((line) => JSON.parse(line));
