...
- Creator transition and STORM evidence tools are canonical artifact-only/read-only intelligence tools. Use `npm run transition:report -- --period monthly --from 2017-11-25 --to 2026-06-24 --out .tmp/transition/latest` and `npm run storm:evidence -- --transition-artifact .tmp/transition/latest/states.json --out .tmp/storm/latest` from `/opt/crypto-tuber-ranked`. They must not write DB rows, publish, call providers, or score creators. Altcoin Daily is news/media context only and must not be measured as a creator.
