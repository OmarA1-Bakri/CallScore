  ...
  if (session instanceof Response) {
    return (
      <SettingsShell title="Alerts">
        <p className="font-serif text-[18px] text-ink-700">Pro unlocks watchlists and email alerts.</p>
        <Link href="/pricing" className="text-accent font-mono text-[11px] tracking-caps uppercase">Upgrade</Link>
      </SettingsShell>
    );
  }
  ...
  return (
    <SettingsShell title="Alerts">
      <section className="border-y border-ink-150 py-4">
        ...
        <h2 className="font-mono text-[11px] tracking-caps uppercase text-ink-500 mb-3">Watchlist</h2>
        {watches.length === 0 ? (
          <p className="text-ink-500 text-sm">No watched creators yet.</p>
        ) : (
          <ul className="space-y-2">
            {watches.map((watch) => (
              <li key={watch.id} className="font-mono text-[12px] text-ink-700">
      ...
      <section className="py-4">
        <h2 className="font-mono text-[11px] tracking-caps uppercase text-ink-500 mb-3">Recent alert queue</h2>
        {recentAlerts.length === 0 ? (
          <p className="text-ink-500 text-sm">No recent alerts.</p>
        ) : (
          <ul className="space-y-2">
            {recentAlerts.map((alert) => (
              <li key={alert.id} className="font-mono text-[12px] text-ink-700">
...
function SettingsShell({ title, children }: { readonly title: string; readonly children: ReactNode }) {
  return (
    <main className="max-w-page mx-auto px-4 tab:px-6 desk:px-8 py-10">
      <p className="font-mono text-[10px] text-ink-500 tracking-caps uppercase mb-2">settings</p>
      <h1 className="font-serif text-[34px] text-ink-900 font-medium mb-6">{title}</h1>
